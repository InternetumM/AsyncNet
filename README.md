...
Now you just have to encode/frame (prepend your message with a four byte integer) any outgoing messages and decode (skip the frame header - four byte integer) any incoming frames to get your message. This simple class here will do the job:
```csharp
...
It is sometimes needed to switch a protocol to a different one for a particular peer at some point in time. 
